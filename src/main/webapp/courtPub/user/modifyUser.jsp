...
function verify(){
	...
	if(true == ajaxData){
		$.ajax({
			type:"get",
		    async: false,
			url:"<c:url value='/verifyUserName?userName="+userName+"&userId="+id+"'/>",
...
<body>
	<div id="vod_pub_main">
		<div id="head_title" style="width:100%;height: 55px;">
			<span id="add_title"
				style="width: 300px; height: 51px; position: absolute; left: 100px; top: 20px;">更改用户信息</span><br />
			<br /> <br />
			<hr>
		</div>
	<form action="modifyUser" enctype="multipart/form-data" method="post" class="main_form" name="frm"  onsubmit="return validate_channel_info(this);">
		<div style="width:560px;height:400px;">
			<table id="addPub" style="width: 520px;">
				<tr>
					<td align="right">所属法院：</td>
					<td align="left"><input type="text" name="courtName" value="${courtName }" id="court"  readonly="readonly"/></td>
					<td width="60px">
				...
				<tr>
					...
					<td align="left"><input type="text" name="userName" value="${user.userName }" id="userName" onblur="verify()" maxlength="11"/></td>
					<td>
				...
				<tr>
					...
					<td align="left"><input type="password" name="userPassword" value="${user.userPassword }" id="passWord" maxlength="10"/></td>
					<td></td>
				</tr>
				<tr>
					<td align="right">真实姓名：</td>
					<td align="left"><input type="text" name="realName" value="${user.realName }" id="" maxlength="7"/></td>
					<td>
						<input type="hidden" name="createBy" value="${user.createBy }"  />
						<input type="hidden" name="createTimed"
